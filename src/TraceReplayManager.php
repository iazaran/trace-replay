...
class TraceReplayManager
{
    ...
    {
        try {
            if (config('tracereplay.queue.enabled') && class_exists(\TraceReplay\Jobs\PersistTraceStepJob::class)) {
                dispatch(new \TraceReplay\Jobs\PersistTraceStepJob($step->toArray()))
